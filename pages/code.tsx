import { GetStaticProps, InferGetServerSidePropsType } from 'next'
import Image from 'next/image'
import { motion } from 'framer-motion'

import Page from 'components/Page'
import { fetchEntriesCode } from 'utils/contentfulPosts'
import { fadeIn, stagger } from 'components/Animations/Motion'

const Code: React.FC = ({ code }: InferGetServerSidePropsType<typeof getStaticProps>) => {
  return (
    <Page title='Code'>
      <motion.ul className='' animate='animate' initial='initial' exit={{ opacity: 0 }} variants={stagger}>
        {code && code.length > 0 ? (
          code.map((item: { fields: P; sys: { id: string } }) => <CodeItems key={item.sys.id} data={item.fields} />)
        ) : (
          <p>Nothing to see here</p>
        )}
      </motion.ul>
    </Page>
  )
}

interface Props {
  data: P
}
interface P {
  image: [{ secure_url: string; width: number; height: number }]
  title: string
  description: string
  delivered: string
  gitLink: string
  link: string
}

const CodeItems: React.FC<Props> = ({ data }) => {
  interface GitProp {
    link: string
  }
  const GitLink: React.FC<GitProp> = ({ link }) => {
    if (link === '/') {
      return null
    } else {
      return (
        <a href={link} target='_blank' rel='noreferrer'>
          <svg
            xmlns='http://www.w3.org/2000/svg'
            className='h-5 w-5'
            fill='currentColor'
            viewBox='0 0 24 24'
            stroke='none'
            role='img'
          >
            <path
              d='M12 .297c-6.63 0-12 5.373-12 12 0 
            5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 
            3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 
            1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 
            1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 
            1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 
            24 17.592 24 12.297c0-6.627-5.373-12-12-12'
            />
          </svg>
        </a>
      )
    }
  }
  return (
    <motion.li className='mb-10 rounded-xl bg-gray-400 dark:bg-gray-800 shadow-lg relative' variants={fadeIn}>
      <a href={data.link} target='_blank' rel='noreferrer'>
        <Image
          className='rounded-t-xl'
          src={data.image[0].secure_url}
          alt={data.title}
          width={data.image[0].width}
          height={data.image[0].height}
          objectFit='contain'
        />
      </a>

      <motion.div className='p-6 pt-3 sm:p-10 sm:pt-5'>
        <h3 className='mb-2 tracking-normal'>{data.title}</h3>
        <p className='text-gray-400 leading-snug'>{data.description}</p>
        <br />
        <div
          className='py-2 px-3 flex justify-between items-center font-mono font-medium dark:text-gray-700 
        bg-yellow-400 border-gray-900 border-2 rounded-md'
        >
          <div className=''>
            <p>{data.delivered}</p>
          </div>
          <div className='flex items-center'>
            <GitLink link={data.gitLink} />
            <a className='ml-1' href={data.link} target='_blank' rel='noreferrer'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                className='h-6 w-6'
                fill='none'
                viewBox='0 0 24 24'
                stroke='currentColor'
              >
                <path
                  strokeLinecap='round'
                  strokeLinejoin='round'
                  strokeWidth={2}
                  d='M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14'
                />
              </svg>
            </a>
          </div>
        </div>
      </motion.div>
    </motion.li>
  )
}

export default Code

interface Index {
  fields: []
}

export const getStaticProps: GetStaticProps = async () => {
  const data: Index[] | undefined = await fetchEntriesCode()

  return {
    props: {
      code: data ?? null,
    },
    revalidate: 1,
  }
}
